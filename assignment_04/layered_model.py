...
def define_dense_model_single_layer(input_length, activation_f='sigmoid', output_length=1):
    ...
    model = tf.keras.Sequential([
        tf.keras.layers.Dense(output_length, activation=activation_f, input_shape=(input_length,))
    ])
...
def define_dense_model_with_hidden_layer(input_length, 
    ...
    model = tf.keras.Sequential([
        tf.keras.layers.Dense(hidden_layer_size, activation=activation_func_array[0], input_shape=(input_length,)),
...
def evaluate_mnist_model_single_digit(x_test, y_test, target_digit, model):
    ...
    loss, accuracy = model.evaluate(x_test, y_test, verbose=False) ## change this.
